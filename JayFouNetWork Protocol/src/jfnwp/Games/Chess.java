...
public class Chess extends Game {
	...
	public void applyMove(IMove m) throws MoveException {
		...
		if (checkMove(mo)) {
			...
			if (isMat(col)) {
				...
				sendResult(mo.toString(),
						getNextPlayerToMoveIp(mo.getPlayerIp()));
	...
	public boolean freeRoad(ChessMove mv) {
		...
		while (x != 0 || y != 0) {
			...
			if (!noPiece(posTmp)
					&& !((posTmp.getX() == from.getX()) && (posTmp.getY() == from
							.getY())) && !((x == 0) && (y == 0))) {
				return false;
			}
	...
	public Boolean becomeMat(ChessMove mv) {
		...
		for (i = 0; i < 8; i++) {
			for (j = 0; j < 8; j++) {
				if ((chessTmp.board[i][j] instanceof King)
						&& chessTmp.board[i][j].getColor().equals(kingColor)) {
		...
		for (i = 0; i < 8; i++) {
			for (j = 0; j < 8; j++) {
				if (board[i][j] != null) {
					...
					if (!pTmp.getColor().equals(piece.getColor())
							&& pTmp.checkMove(mv, this))
						return true;
				}
			}
		}
	...
	public Boolean isMat(Color color) {
		...
		ChessPiece piece = (ChessPiece) board[kingPosition.getX()][kingPosition
				.getY()];
